...
| R10 | Perda de dados (corrupção/exclusão do SQLite, falha da VM Free Tier — sem SLA) | Continuidade | Média | Alto | **Alto** | Backup automatizado, criptografado, fora do servidor (ver `backup-recovery.md`) | 🟡 Implementado no código (ADR-030), pendente só de configurar as credenciais do R2 no ambiente de produção |
