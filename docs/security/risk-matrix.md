...
| R12 | Exceção não tratada expõe detalhe interno (falha na chamada ao Turnstile ou ao SMTP) | A10 | Média | Médio | **Médio** | `DEBUG=False`, `try/except` ao redor de chamadas externas, página de erro genérica | 🟡 Parcial — Turnstile coberto (`TurnstileService`, falha fechado); envio de e-mail ainda pendente |
...
| 2b | Parede de 2FA (TOTP/e-mail) com alerta por e-mail em código incorreto | Fechar R03; é o requisito funcional que diferencia o projeto | `apps/accounts/views.py`, `templates/accounts/` | Próxima sprint | Aluno (dev) | `TwoFactorService` (verificação TOTP/e-mail via `pyotp`) + envio de e-mail via conta Gmail dedicada | ~4h, $0 |
